...
class ITFile:
    ...
    def doheader(self, fp):
        ...
        fp.write(struct.pack("<HHHHHHHHH", self.highlight, len(ordlist), len(self.inslist), len(self.smplist), len(self.patlist), self.version, self.vercompat, self.flags, self.special
                             ))
        fp.write(struct.pack("<BBBBBBH", self.gvol, self.mvol, self.speed, self.tempo, self.pansep, self.pitchwheel, len(self.message_fixed)
                             ))
...
class Sample:
    ...
    def write(self, fp):
        ...
        fp.write(struct.pack("<IIIIII", self.length, self.lpbeg,
                             self.lpend, self.freq, self.susbeg, self.susend))
        fp.enqueue_ptr(self.write_data)
        fp.write(struct.pack("<BBBB", self.vibspeed,
                             self.vibdepth, self.vibrate, self.vibtype))
...
class Pattern:
    ...
    def __init__(self, rows):
        self.rows = rows
        # note, this is just so modplug doesn't whinge. IT can handle 1-row
        # patterns.
        assert rows >= 4, "too few rows"
        # on the other hand, IT chunders if you have more than 200 rows.
        assert rows <= 200, "too many rows"
        self.data = [[[253, 0, 255, 0, 0]
                      for j in xrange(64)] for i in xrange(rows)]
...
class Sample_Hoover(Sample):
    ...
    def generate(self, freq):
        oscfrq = [
            int(freq * (v + v * (random.random() * 2.0 - 1.0) * 0.002)) /
            float(SMP_FREQ)
            for v in [0.25, 0.5, 1.0, 2.0]
        ]

        oscvibspeed = [float(random.randint(1, 5)) * 2.0 *
                       math.pi / SMP_FREQ for i in xrange(4)]
...
class Strategy_Main(Strategy):
    ...
    def newkseq(self):
        self.kseq = random.choice({
            Key_Minor: [
                [(0, Key_Minor), (-4, Key_Major),
                 (5, Key_Major), (-2, Key_Major)],
                [(0, Key_Minor), (-2, Key_Major),
                 (-4, Key_Major), (-5, Key_Minor)],
            ],
            Key_Major: [
                [(0, Key_Major), (-5, Key_Major),
                 (-3, Key_Minor), (5, Key_Major)],
                [(0, Key_Major), (0, Key_Major),
                 (-7, Key_Minor), (-5, Key_Major)],
            ],
        }[self.keytype])
    ...
    def get_pattern(self):
        ...
        for i in xrange(0, self.patsize, self.blocksize):
            ...
            for chn, gen in self.gens:
                gen.apply_notes(chn, pat, self, self.rhythm, i,
                                self.blocksize, self.key, kchord)
                ...
                if leadin != 0 and random.random() < 0.4:
                    ...
                    for j in xrange(count):
                        pat.data[row - (j + 1) * gran][chn] = [
                            base + 12 if random.random() < 0.5 else base, self.smp, 0xFF, ord('S') -
                            ord('A') + 1, 0xC0 + random.randint(1, 2)
                        ]
    ...
    def apply_notes(self, chn, pat, strat, rhythm, bbeg, blen, kroot, kchord):
        ...
        while row < bbeg + blen:
            ...
            if self.mq:
                ...
                backstep = random.randint(
                    3, min(10, row // (self.beatrow // 2))) * (self.beatrow // 2)
                ...
                while True:
                    ...
                    for j in xrange(20):
                        ...
                        down = random.random() < (8.0 + (self.ln - base)) / \
                            8.0 if self.ln != -1 else 0.5
...
# note: commented a couple out as they use a fair whack of space and are
# unused.
SMP_GUITAR = itf.smp_add(Sample_KS(name="KS Guitar", freq=MIDDLE_C / 2, decay=0.005,
                                   nfrqmul=1.0, filt0=0.1, filtn=0.6, filtf=0.0004, length_sec=1.0))
SMP_BASS = itf.smp_add(Sample_KS(name="KS Bass", freq=MIDDLE_C / 4, decay=0.005,
                                 nfrqmul=0.5, filt0=0.2, filtn=0.2, filtf=0.005, length_sec=0.7))
...
SMP_HHC = itf.smp_add(Sample_NoiseHit(
    name="NH Hihat Closed", gvol=32, decay=0.03, filtl=0.99, filth=0.20))
SMP_HHO = itf.smp_add(Sample_NoiseHit(
    name="NH Hihat Open", gvol=32, decay=0.5, filtl=0.99, filth=0.20))
SMP_SNARE = itf.smp_add(Sample_NoiseHit(
    name="NH Snare", decay=0.12, filtl=0.15, filth=0.149))


print("Generating patterns")
strat = Strategy_Main(random.randint(50, 50 + 12 - 1) + 12,
                      Key_Minor if random.random() < 0.6 else Key_Major, 128, 32)
strat.gen_add(Generator_Drums(s_kick=SMP_KICK,
                              s_snare=SMP_SNARE, s_hhc=SMP_HHC, s_hho=SMP_HHO))
...
mynotes = ['C0', 'C#0', 'Db0', 'D0', 'D#0', 'Eb0', 'E0',
           'F0', 'F#0', 'Gb0', 'G0', 'G#0', 'Ab0', 'A0',
           'A#0', 'Bb0', 'B0', 'C1', 'C#1', 'Db1',
           'D1', 'D#1', 'Eb1', 'E1', 'F1', 'F#1', 'Gb1',
           'G1', 'G#1', 'Ab1', 'A1', 'A#1', 'Bb1', 'B1',
           'C2', 'C#2', 'Db2', 'D2', 'D#2', 'Eb2', 'E2',
           'F2', 'F#2', 'Gb2', 'G2', 'G#2', 'Ab2', 'A2',
           'A#2', 'Bb2', 'B2', 'C3', 'C#3', 'Db3', 'D3',
           'D#3', 'Eb3', 'E3', 'F3', 'F#3', 'Gb3', 'G3',
           'G#3', 'Ab3', 'A3', 'A#3', 'Bb3', 'B3', 'C4',
           'C#4', 'Db4', 'D4', 'D#4', 'Eb4', 'E4', 'F4',
           'F#4', 'Gb4', 'G4', 'G#4', 'Ab4', 'A4', 'A#4',
           'Bb4', 'B4', 'C5', 'C#5', 'Db5', 'D5', 'D#5',
           'Eb5', 'E5', 'F5', 'F#5', 'Gb5', 'G5', 'G#5',
           'Ab5', 'A5', 'A#5', 'Bb5', 'B5', 'C6', 'C#6',
           'Db6', 'D6', 'D#6', 'Eb6', 'E6', 'F6', 'F#6',
           'Gb6', 'G6', 'G#6', 'Ab6', 'A6', 'A#6', 'Bb6',
           'B6', 'C7', 'C#7', 'Db7', 'D7', 'D#7', 'Eb7',
           'E7', 'F7', 'F#7', 'Gb7', 'G7', 'G#7', 'Ab7',
           'A7', 'A#7', 'Bb7', 'B7', 'C8', 'C#8', 'Db8',
           'D8', 'D#8', 'Eb8']
...
RN_NOUNS = [
    ...
    ("city", "cities"), ("castle", "castles"), ("town",
                                                "towns"), ("village", "villages"),
...
name = randoname()
itf.name = name
fname = "bu-%s" % name.replace(" ", "-").replace("'", "")
if len(sys.argv) > 1:
    fname = sys.argv[1]
itf.save(fname + ".it")
